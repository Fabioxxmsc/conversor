...
class Config():
    ...
    def TesseractPath(self):
        if self.__tesseractPath is None:
            self.__tesseractPath = self.ReadIni(
                'tesseract', 'path', PATH_DEFAULT_TESSERACT)
        return self.__tesseractPath

    def TesseractThreads(self):
        if self.__tesseractThreads is None:
            self.__tesseractThreads = int(
                self.ReadIni('tesseract', 'threads', '1'))
    ...
    def DataSetPath(self):
        if self.__dataSetPath is None:
            self.__dataSetPath = self.ReadIni(
                'dataset', 'path', PATH_DEFAULT_DATASET)
    ...
    def ConnectionInfo(self) -> ConnectionInfo:
        ...
        if self.__connectionInfo.host == '':
            self.__connectionInfo.host = self.ReadIni(
                'connection', 'host', 'localhost')
            self.__connectionInfo.database = self.ReadIni(
                'connection', 'database', 'postgres')
            self.__connectionInfo.user = self.ReadIni(
                'connection', 'user', 'postgres')
            self.__connectionInfo.password = self.ReadIni(
                'connection', 'password', '')
