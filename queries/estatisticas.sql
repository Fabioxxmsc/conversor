...
-- Tempo minnímo, médio e máximo de duração por processamento de documento
...
select (select concat('M�dia: ', round(avg(e.acerto)::numeric, 0), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto = 100) AS DE_100_A_100
     , (select concat('M�dia: ', round(avg(e.acerto)::numeric, 4), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto between 90 and 99.9999) AS DE_90_A_99
     , (select concat('M�dia: ', round(avg(e.acerto)::numeric, 4), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto between 80 and 89.9999) AS DE_80_A_89
     , (select concat('M�dia: ', round(avg(e.acerto)::numeric, 4), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto between 70 and 79.9999) AS DE_70_A_79
     , (select concat('M�dia: ', round(avg(e.acerto)::numeric, 4), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto between 60 and 69.9999) AS DE_60_A_69
     , (select concat('M�dia: ', round(avg(e.acerto)::numeric, 4), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto between 50 and 59.9999) AS DE_50_A_59
     , (select concat('M�dia: ', round(avg(e.acerto)::numeric, 4), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto between 40 and 49.9999) AS DE_40_A_49
     , (select concat('M�dia: ', round(avg(e.acerto)::numeric, 4), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto between 30 and 39.9999) AS DE_30_A_39
     , (select concat('M�dia: ', round(avg(e.acerto)::numeric, 4), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto between 20 and 29.9999) AS DE_20_A_29
     , (select concat('M�dia: ', round(avg(e.acerto)::numeric, 4), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto between 10 and 19.9999) AS DE_10_A_19
     , (select concat('M�dia: ', round(avg(e.acerto)::numeric, 4), '%, Contagem: ', count(e.acerto)) from estatistica e where e.acerto between 0 and 9.9999) AS DE_0_A_9
  from combinacoes c 
...
-- Combina��es que tiveram 100%, ordenados pela maior quantidade de ocorr�ncia 
...
-- Total de valores extra�dos dos documentos
select count(*)
  from documentovalor dv;

-- Todos os registros de valores extra�dos dos documentos
